...
public class Cties {
	public static void main(String[] args) {
		...
		TreeMap<String, Integer> Cties = new TreeMap<>();//TreeMap to sort keys in alphabetical order

		Cties.put("London", 6);
		Cties.put("Dallas", 6);
		Cties.put("Istanbul", 8);
		Cties.put("Paris", 5);
		Cties.put("Washington DC", 13);
		Cties.put("Waco", 6);
		Cties.put("Izmir", 6);
		                        System.out.println(Cties);
		                                                                                                            System.out.println("  ");
         //remove entries  : using    .entrySet()
		Collection<Entry<String,Integer>> key=Cties.entrySet();//Storing Map into collection: using Map.EntrSet()
		...
		System.out.println(Cties);
		                                                                                                           ...
		                                                                                                           System.out.println("   ");
		Collection<String> Key = Cties.keySet(); // Storing Map into collection: using Map.keySet()

		Iterator<String> itt = Key.iterator();
		while (itt.hasNext()) {
			if (itt.next().length() > 7) {// remove :using .keySet()
				itt.remove();
			}
		}
